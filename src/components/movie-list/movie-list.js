...
export const MovieList = (props) => {
  ...
  return (
    <div className="movie-list">
      {fetchFailed ? (
        <Alert message="Loading error" description="Please, turn on VPN and reload site" type="error" closable />
      ) : isLoading ? (
        <Spin></Spin>
        ...
        <div className="movie-list-box">{createFullMovies()}</div>
      )}
      <MovieListPagination
        current={isRated ? ratedCurrPage : currPage}
        totalPages={isRated ? ratedTotalPages : totalPages}
      ></MovieListPagination>
    </div>
  )
};
